...
const TUDashboard = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const { data, error } = await supabase
        ...
        `)
        .in('status', ['submitted', 'reviewed_by_tu'])
  ...
  const handleReview = async (submissionId: string, action: 'approve' | 'reject') => {
    ...
    try {
      const status = action === 'approve' ? 'approved_by_tu' : 'rejected_by_tu';
      
      const { error } = await supabase
        .from('document_submissions')
        .update({
          status,
          tu_review_notes: reviewNotes,
          tu_reviewed_at: new Date().toISOString(),
          tu_reviewed_by: user.id,
        })
        .eq('id', submissionId);
      ...
      toast({
        title: "Sukses",
        description: `Pengajuan berhasil ${action === 'approve' ? 'disetujui' : 'ditolak'}`,
      });
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Dashboard TU</h2>
        <p className="text-gray-600">Review dan kelola pengajuan surat mahasiswa</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {submissions.filter(s => s.status === 'reviewed_by_tu').length}
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        {submissions.length === 0 ? (
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-gray-500">Tidak ada pengajuan surat</p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  <div className="space-y-4">
                    ...
                    {submission.tu_review_notes && (
                      <div className="bg-yellow-50 p-3 rounded-md">
                        <p className="text-sm font-medium text-yellow-800">Catatan Review:</p>
                        <p className="text-sm text-yellow-700">{submission.tu_review_notes}</p>
                      </div>
                    )}
                    ...
                    <div className="flex space-x-2">
                      <Button 
                        ...
                        onClick={() => {
                          ...
                          setReviewNotes(submission.tu_review_notes || '');
                        }}
                      >
